...
## 3、异质性
在联邦学习的应用场景中，设备的差异会使整个训练过程的效率低下。为了解决系统异质问题，有四种方式：异步通信、设备采样、容错机制和模型异质。异步通信即异步联邦学习；设备采样即设备选择。
为了解决（模型异质性）统计数据异质的问题，联邦学习网络主要分为三种建模方法：
单个设备有自己的模型，个性化联邦学习
训练适用于所有设备的全局模型
为任务训练相关的学习模式
## 4、数据异质性
为了训练适用于所有设备的全局模型，需要充分利用各个联邦学习客户端的数据，同时需要应对联邦学习面临的一大难题，数据异质性问题。
### FedAvg
17年谷歌提出的联邦学习概念及其算法，局部训练和聚合算法相对简单，聚合算法为每个客户端数据量占比与模型参数的乘积求和，得到全局模型。

### FedProx
通过在局部训练的损失函数中增加约束项，使得局部模型训练过程中不至于偏移全局模型太多，为每个当前局部模型参数和本轮全局模型参数计算2范数并求和
![FedProx](conclusion/FedProx.png)
聚合采用的是等权重1/K聚合。
...
## 7、方向
    ...
    4、设备选择：如何在联邦学习任务中找到值得信赖和可靠的客户端是至关重要的。一些论文中引入了声誉的概念，作为衡量客户可靠性的指标，也可以与区块链结合。
## 8、总结
一方面，当数据不足导致用户无法训练出满意的模型时，联邦学习可以聚合多方用户模型，在不暴露原始数据的情况下对集成的模型进行更新。
另一方面，当用户没有足够的数据标签学习时，联邦学习不仅可以为用户提供安全的模型共享机制，还可以将模型迁移到特定的任务中，解决数据标签不足的问题。
...
基于策略的强化学习不再通过价值函数来确定选择动作的策略，而是直接学习策略本身，通过一组参数$\theta$对策略进行参数化，并通过BP神经网络方法优化策略网络$\theta$

## 策略梯度算法
主要区别：基于策略的强化学习用参数化概率分布$\pi_\theta(a|s)=P(a|s,\theta)$代替了基于值函数的强化学习中的确定性策略$\phi:s-> a$ ，在返回的动作概率列表中对不同的动作进行抽样选择。
基于策略梯度理论如下
